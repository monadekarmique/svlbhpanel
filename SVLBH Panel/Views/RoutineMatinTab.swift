    ...
    private var tableauCertifiees: some View {
        VStack(alignment: .leading, spacing: 8) {
            ...
            HStack(spacing: 0) {
                ...
                Text("Max").font(.caption2.bold()).foregroundColor(.secondary).frame(width: 54, alignment: .trailing)
                Text("Cpt").font(.caption2.bold()).foregroundColor(.secondary).frame(width: 54, alignment: .trailing)
                Text("Libre").font(.caption2.bold()).foregroundColor(.secondary).frame(width: 58, alignment: .trailing)
                Text("%").font(.caption2.bold()).foregroundColor(.secondary).frame(width: 56, alignment: .trailing)
            }
            ...
            ForEach(certifiees) { q in
                HStack(spacing: 0) {
                    ...
                    .lineLimit(1)
                    .frame(maxWidth: .infinity, alignment: .leading)
                    Text("\(q.max)").font(.system(.caption2, design: .monospaced).bold())
                        .lineLimit(1).frame(width: 54, alignment: .trailing)
                    Text("\(q.compteur)").font(.system(.caption2, design: .monospaced))
                        .lineLimit(1).frame(width: 54, alignment: .trailing)
                    Text("\(q.quotaLibre)")
                    ...
                    Text(String(format: "%.0f%%", q.pourcentage))
                        .font(.system(.caption2, design: .monospaced).bold())
                        .foregroundColor(q.quotaLibre >= 0 ? Color(hex: "#1D9E75") : Color(hex: "#E24B4A"))
                        .lineLimit(1).frame(width: 56, alignment: .trailing)
                }
